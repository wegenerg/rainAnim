...
void updateCoordFrames()
{
    ...
    if (is_anim_running) {
        ...
        for(int i = 0; i < STORMSIZE; i++){
            rainArray[i].xCoord += windX;
            rainArray[i].yCoord += windY;
            rainArray[i].zCoord -=.25;
...
void displayCallback (GLFWwindow *win)
{
    ...
    glPushMatrix();
    {
        glMultMatrixf(glm::value_ptr(rain_drop_cf));
        drop.render();
    }
    glPopMatrix();
